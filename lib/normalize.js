...
export const normalizeComponent = (component) => {
  ...
  for (const index in flattenChildren(children)) {
    const { key, component: input } = children[index]

    if (key && keyedChildren[key] !== undefined) throw Error('Duplicate keys detected.')

    if (key) keyedChildren[index] = { key, index, input }
    else keyedChildren[index] = { key: 'bitt-' + index, index, input: children[index] }
  }
